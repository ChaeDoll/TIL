...
## 8주차 수업 정리  
혈액형, 직업같은 데이터들은 분류에 도움은 줄 수있지만 계량 데이터 점수, 속도처럼 실제적인 거리개념이 없기에 분류 어려움  

결정트리는 스무고개와 유사함  

불순도의 감소량이 가장 크게되는 형태를 부류로 선택하는 것이 기본적인 원리이다.  

노드의 불순도가 작냐크냐 수식을 통해 구하고,  
트리의 왼쪽, 오른쪽에서의 불순도를 구한다.  
그리고 이들을 통해 불순도 감소량을 계산한다.  

노드마다 그 노드 내의 원소 중 가장 주가 되어 이루는 부류를 그 노드의 대표 부류로 지정.  

가지치기가 가능하다.  
사전 가지치기처럼 단계단계 내려가더라도 결정을 변하게.. 이게 결정트리 주로 사용?  
한번에 하나의 질문으로 하는것이기에 안정성이 완전히 높진 않다. 