...
- scale out의 단점은 더욱 복잡하다. ScaleUp은 세션, 토큰, 유저데이터 등 다양한 정보를 그냥 다 가지고있으면된다. 근데 scale out은 여러 서버에서 이런 상태를 가지고있지않아 에러가 날 수 있다. 그래서 서버를 Stateless 서버로 만들어야 한다. (필요할 때 가져와 사용하는 방법. 무상태)
- 