...
- 모델 : llama 3.1사용. Temperature옵션은 0을 사용하여 답변에 대한 창의성을 낮추었다. 

**결과**
가끔 한문이 튀어나오거나 한글이 어색하거나 하곤했었다.  라마3로도 히스토리챗봇을 구현할수있었다. 
- AdvancedRAG사용하니까 결국 NativeRAG문제점 해결이 가능했음.
- 또한 보안적 이슈 해결되었음
- LLM 베이스로 하는 다양한 기능제작이 가능하다
- 여러 분야 응용이 가능하다
	- Q&A, 요약, 코딩어시스턴트 등..
- 현대오토에버에도 이걸 도입하고자 하고있음 현재 프로젝트로 진행하고있음
- 스터디는 1시간씩했는데 왕복 3시간이 걸렸다. 그래도 좋은사람들 만날수있어서 좋았고, 생소했지만 기술적 발전을 이루었고, 동기부여가 되었고, 앞으로 나의 방향성을 알 수 있었다.

**질의응답**
- 임베딩 모델의 차원수가 성능에 영향을 미칠까요? / 
- 벡터DB중 RAG로 사용하기 가장 좋은 DB는 어떤 것이라고 생각하나요? / 전부는 못써봤지만, 파인포어?를 많이 쓰는것같다. OpenSearch 추천 좀 한다. 메타데이터나 라벨링하기 좋다. 이 기능으로 RAG단점을 보완할수있지않을까.
- LLM을 라마3썼다했는데, RAG가 LLM에 따라 응답정확도 달라지는지? / 모델 3~4개 써봤을때, 모델에 따라 답변 달라지기보단 환경에 따라 달라지더라. CPU냐 GPU냐에 시간이 많이차이..
- Temperature을 왜 0으로? / RAG여서 0으로한것보다, 스터디에 먼저 값을 0으로 주는것을 통일했다. 일단 일정한 값을 내놓는것을 우선했다. 평가하기위해서도.
- GPU가 없는 리눅스서버에선 맥북보다 처리 속도가 느릴것같은데 현실적으로 동작이 어려울까요? / Gemma는 온디바이스 있긴한데 맥북보다 느리긴한데 동작은가능하다.
- LLM이 영어기준으로 만들었다해서 한글형태소 분석을 넣어서 제작했는데, 한글로 RAG할때 발생했던 문제 그리고 해결 사례 / 이걸 적용해서인지 아닌지는 모르겠는데, 처음보다 말이 어순이 한글스럽게 나왔다고 생각한다. 좀 더 자연스럽게 표현되었다.
- 사용한 RAG기법중 가장 체감효과 뛰어났던것? / 개인적으로는 HybridSearch(앙상블)가 좋았다 생각한다. GPU에서 정확히 알수있겠지만 멀티쿼리는 부하가 걸리는 경우가있었다.
- 많은 문서 넣어서도 해봤는지 / 최대 300페이지
- ReRank 유무가 결과값 정확도 차이가 큰가요? / 300개 페이지를 넣었을때 불필요함이 나타나기했는데 그게 사라지더라
- 어떤것을 기준으로 최적화를 진행했는지... / 벤치마크 데이터셋 사용은 못했다. 벤치마크는 모델을 선정하는 기준에서 봤고.. 최적화는 소형 모델이며 컨텍스트기대가 크지않고 chunk사이즈를 작게하여 진행? 가장 중요했던건 Retrieval 기법에 대해..
- ReRank모델은 어떤것을 사용? / ReRank 기법에 대해 공부해서 그것과 비슷하게 동작하는 함수를 만들어서 적용했다.